...
**Regla de costo:**
- **`item_producto.costo_actual` es un promedio ponderado móvil (CPP), no el último costo.** Lo recalculan la entrada por **compra** y —desde el 2026-08-22— las entradas que **revierten** una salida (`anulacion`, `devolucion`), con la misma fórmula: `(stock_previo × costo_actual_previo + cantidad × costo_compra) / (stock_previo + cantidad)`, redondeado a 4 decimales. `stock_previo` es el stock del producto en **todas** sus ubicaciones activas, no el `stock_anterior` del kardex, que es el de la ubicación del movimiento: el costo es uno solo por producto (decisión 3 de [`bodegas-y-traslados.md`](bodegas-y-traslados.md)). Hasta el 2026-09-18 pesaba el de la ubicación, y 10 kg comprados en un local vacío pisaban el costo de los 100 kg de la bodega. Sin stock previo o sin costo previo (no hay masa que promediar, y evita dividir por cero), el costo de compra manda tal cual. Implementado en `InventarioService.calcularCostoPromedio` (privado) y cableado en `registrarMovimiento` vía la variable `costoActualNuevo` (`string | null`; `null` = no se toca `item_producto`).
