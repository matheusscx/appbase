...
- `composables/useVenta.ts` — `ItemCatalogo.disponible?: number | null`. El descuento local del carrito (`descontarStockCatalogo`) **conserva el signo**, igual que `stockDisponible` y al revés que `stock`: pisarlo a 0 escondía el negativo que la API manda a propósito apenas el vendedor agregaba el primer ítem (corregido el 2026-09-02).
