...
**Regla "exento" (clasificación tributaria del item):** un item puede marcarse `afecto` (default) o `exento`. `exento` suprime **únicamente** el IVA de esa línea; los impuestos adicionales (`item_impuestos`, siempre `tipo = 'otro'`) siempre se aplican, esté o no exento. La clasificación se **congela por línea de venta** (`venta_detalles.clasificacion_tributaria`) en el momento de vender — no se recalcula si el item cambia de clasificación después, y una nota de crédito hereda la clasificación congelada de la línea original, no la del item vigente. Su **línea de ajuste** —la parte del monto que no corresponde a mercadería devuelta— no copia ninguna línea: lleva la clasificación de la **porción** que expresa, y por eso una nota sobre una venta mixta sale con dos líneas de ajuste, una afecta y una exenta, repartidas en la proporción de lo que queda por acreditar — **salvo que una porción no tenga remanente, o que el reparto la deje en cero: ahí sale una sola línea**, porque una línea de importe cero es ruido en el documento y puede no ser válida al emitirlo.
