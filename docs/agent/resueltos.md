...
día que revirtió la regla contraria que él había aprobado horas antes—. Investigación en
[`2026-09-04-devolucion-con-credito-parcial.md`](investigaciones/2026-09-04-devolucion-con-credito-parcial.md),
diseño en
[`2026-09-04-devolucion-con-credito-parcial-design.md`](../superpowers/specs/2026-09-04-devolucion-con-credito-parcial-design.md),
plan en
[`2026-09-04-devolucion-con-credito-parcial.md`](../superpowers/plans/2026-09-04-devolucion-con-credito-parcial.md).

**Cómo se construyó.** Cinco commits sobre `main`, cada uno dejando el sistema entero:

| Commit | Qué cierra |
|---|---|
| `ead5849c` | `escalarDevoluciones`, pura y testeada: baja las líneas a prorrata con `repartirProporcional` para que sumen, como máximo, el monto |
| `f701b7c1` | **cualquier ítem vendido se acredita por línea**, reponga o no el stock; la reposición pasa a ser propiedad de cada línea |
| `767c5227` | **acreditar menos de lo que vale la mercadería deja de rechazarse**: las líneas se escalan y el motivo pasa a ser obligatorio |
| `e965903e` | `GET /ventas/:id` dice cuánto queda por acreditar, **por porción fiscal** |
| `7fe7046b` | la pantalla: el disponible por porción, el switch de reponer por fila, el motivo pedido |

**Lo que quedó construido, en una línea cada cosa:**

- `devoluciones` significa *"ítems que se acreditan"*, no *"ítems a devolver a stock"*. Los dos
  cortes de inventario disparan según lo que el camino pida del stock, con **tres políticas**
  (`rechazar-imposible` para la nota manual, `ignorar` para el webhook —donde un throw pierde el
  evento— y `exigir` para la devolución sin documento).
- Cuando lo devuelto vale más que el monto, las líneas se **escalan** y la que queda en cero no
  se escribe. El motivo es obligatorio y viaja **pegado al nombre del ítem** en la línea.
- **De los rechazos anteriores sobrevive uno solo**: el tope por porción fiscal, evaluado sobre
  las líneas ya escaladas. Sobrevive porque es invariante fiscal —sin él la serie acredita más
  IVA del que la venta cobró— y no preferencia de producto. El frente **agrega dos propios**, los
  dos del camino manual: motivo faltante cuando la nota escala, y pedir reponer algo que no
  puede.

**Dos bugs que la revisión independiente encontró y que el plan no anticipaba**, los dos
introducidos por el cambio mismo:

1. **El tope por cantidad se quedó ciego.** Contaba `movimientos_inventario`, y eso era fiel
   mientras toda línea aceptada moviera stock. Con líneas que se acreditan **sin** reponer, dos
   notas seguidas podían acreditar la misma receta: el documento afirmaba que volvieron 2
   unidades de una venta de 1. El tope por porción no lo tapa —mira plata, no cantidad—. Hoy
   `unidadesComprometidasPorItem` toma, **por documento**, el mayor entre lo acreditado en líneas
   y lo movido en stock; mayor y no suma, porque la línea que repone deja las dos huellas.
2. **`registrarDevolucionesPorReembolso` ignoraba el flag que se acababa de crear**: reponía lo
   que se pidió no reponer, y un servicio sin flag llegaba hasta `registrarMovimiento` a reventar
   con un error genérico **después** de haber escrito las líneas anteriores del loop. De ahí sale
   la tercera política (`exigir`).

**Y dos en la pantalla**, encontrados por la misma vía:

3. **El gesto del modal de reembolso.** Tildar "generar nota de crédito", apagar el switch de una
   fila, tipear cantidad y **destildar**: el switch desaparece del DOM, así que ese `false`
   quedaba invisible e irrecuperable y salía en el payload. El 400 llega después del commit del
   reembolso —plata devuelta, mercadería que no vuelve al stock, sin reintento posible—.
4. **El disponible congelado.** Al pasar `disponibleNC` al número del backend, `onNcSuccess` y
   `onAbonoSuccess` quedaron pintando optimista un campo del que ese número ahora depende: el
...
relevados, ninguno liga la devolución física a la nota de crédito.

### El texto con el que entró a `pendientes.md`
...
## La nota de crédito descompone su monto (cerrado 2026-09-04)

Sale de [`pendientes.md` § 3](pendientes.md), donde vivía desde el 2026-08-02 y donde el owner
la decidió el 2026-09-03. Diseño en
[`2026-09-04-nota-credito-descompone-su-monto-design.md`](../superpowers/specs/2026-09-04-nota-credito-descompone-su-monto-design.md),
plan en
[`2026-09-04-nota-credito-descompone-su-monto.md`](../superpowers/plans/2026-09-04-nota-credito-descompone-su-monto.md).

**Cómo se construyó.** Cinco commits sobre `main`, en el orden que deja el sistema entero en
cada paso:

| Commit | Qué |
|---|---|
| `20452024` | `export` de `repartirProporcional` y del tipo `Cuantizador` — el único toque autorizado al motor, y con el docblock de `repartirDescuentoCombo` diciendo por qué **no** es intercambiable |
| `405a4342` | El ítem de sistema **"Ajuste"**: columna marcadora, índice único parcial por tenant, find-or-create, sembrado al crear el tenant y en el seed |
| `e24e94a7` | La aritmética pura: tasa efectiva, descomposición por resta, reparto del ajuste |
| `7a1e934d` | El cableado en la nota de crédito — **la tarea indivisible** |
| — | La pantalla y este cierre |

**Lo que estaba mal, medido antes de tocar nada.** La NC guardaba
`total_bruto = total_final = params.monto` con descuentos, recargos e impuestos en `'0'`, cero
filas en `ventas_impuestos` y `base_ventas_*` en el default de la columna. Y **la entrada no
decía dos cosas** que aparecieron al cruzarla con el código:

- la NC nunca escribía `base_ventas_*` — no llega a propinas, pero sí sale por `GET /ventas/:id`;
- la línea de devolución se valuaba al **precio de lista**, ignorando el descuento que el
  cliente había recibido. Inofensivo solo mientras `total_final` fuera independiente de las
  líneas; se rompe en el momento en que las líneas tienen que sumar el total.

**Cómo quedó.** Todo derivado del documento que corrige, nada recalculado contra el catálogo:
la devolución vale lo que costó en esa boleta (`Σ total_linea / Σ cantidad`), el resto del monto
sale en una o dos líneas del ítem "Ajuste" repartidas en la proporción del remanente, y el neto
y el IVA de cada línea salen de la **tasa efectiva** de esa porción —`Σ impuesto / Σ neto` de la
venta original— con el impuesto **por resta**, que es lo que hace cerrar `neto + impuesto =
bruto` cuando la división no cierra. Detalle funcional en
[`features/reembolsos-nota-credito.md`](../features/reembolsos-nota-credito.md).

**Las tres cosas que solo aparecieron midiendo**, y que la revisión independiente bloqueó en
tres rondas —ninguna la habría encontrado leyendo el diff—:

1. **Línea de nota de crédito en negativo.** El ajuste podía acreditar de una porción más de lo
   que esa porción tenía, y la nota siguiente arrancaba con remanente negativo: importe e
   impuesto negativos, y un `total_impuestos` que ninguna fila de `ventas_impuestos`
   reproducía. El remanente ahora descuenta también lo que la propia nota devuelve.
2. **La serie acreditaba más IVA del que la venta cobró.** El tope global mira el bruto y no ve
   la porción fiscal: una nota por monto libre se comía capacidad afecta y la devolución
   siguiente la volvía a usar. Cada documento cerraba bien por separado. Medido: 1.447 de IVA
   acreditado contra 1.330 cobrado. Hay un **segundo tope, por porción**.
3. **El mismo ítem dos veces en `devoluciones`** se validaba contra el mismo disponible. Era
   pre-existente y "solo" duplicaba el movimiento de stock; con la valuación pasaba a duplicar
   plata. Se rechaza.

⚠️ **Lo que queda abierto, escrito para que no se lea como olvido:**

- **Residuo de cuantización, hasta 2 minor units de IVA.** Cada nota descompone su propio bruto
  y cuantiza a la escala de la moneda, así que partir un bruto en varios documentos acumula
  redondeo: **12,15 % de las series multi-nota acredita 1 o 2 pesos de IVA de más** (medido
  sobre 100.000 series; techo 2, no escala con el monto). Sacarlo exige derivar el neto de cada
  nota contra el remanente de la serie: **decisión del owner, no tomada**.
- **Por el webhook, una devolución que no entra en el documento queda fuera de él** (la plata ya
  volvió por el proveedor y el hook corre post-commit: un throw ahí pierde la nota Y el stock).
  El movimiento de inventario corre igual. **Decisión del agente, pendiente de confirmación.**
- **La última unidad de un ítem cuyo valor unitario no divide exacto puede no entrar** por el
  tope por porción: 1 minor unit, con la vuelta a stock desde Inventario como salida.

**Qué lo fija.** `nota-credito-composicion.spec.ts` (la aritmética sola, con `decimalesMoneda:
0`, la escala que más residuo produce) y `nota-credito-composicion.e2e-spec.ts` (12 casos por el
camino de la app, sobre una venta mixta con proporción despareja). Los mutantes del plan —el
impuesto por multiplicación, el reparto sobre la venta original, el inventario sin el corte, la
valuación al precio de lista, el índice del ítem de sistema— caen todos; el del inventario tumba
cuatro tests a la vez, que es la medida de por qué esa tarea era indivisible.
