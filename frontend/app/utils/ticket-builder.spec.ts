...
describe('buildPrecuentaTicket', () => {
  function precuenta(over: Partial<Parameters<typeof buildPrecuentaTicket>[0]> = {}) {
    return buildPrecuentaTicket({
      emisor: EMISOR,
      mesaNombre: 'Mesa 1',
      cuentaNumero: 2,
      items: [ITEM],
  ...
  it('imprime la cabecera del emisor igual que la boleta (nombre, RUT, dirección)', () => {
  ...
  it('imprime el header de columnas CANT/DESCRIPCIÓN/P.UNIT/TOTAL a 48 caracteres, igual que la boleta', () => {
    const lines = precuenta()
  ...
  it('imprime Neto y una línea por impuesto con nombre y tasa reales, igual que la boleta', () => {
    const lines = precuenta()
    ...
    expect(lines.some(l => l.startsWith('TOTAL') && !l.startsWith('TOTAL BOLETA') && l.includes('$37000'))).toBe(true)
  })

  it('incluye personalización como lista debajo de la fila del ítem', () => {
    const lines = precuenta({
      items: [{ nombre: 'Hamburguesa Clásica', cantidad: '1', precioUnitario: '9800', totalLinea: '9800', nota: 'Extra Queso · Sin Cebolla' }],
    })
  ...
  it('con propinaSugerida imprime el bloque sugerido y la leyenda voluntaria', () => {
    const lines = precuenta({ propinaSugerida: { porcentaje: '0.10', monto: '3700' } })
    expect(lines.some(l => l.startsWith('Propina sugerida 10%') && l.includes('$3700'))).toBe(true)
    expect(lines.some(l => l.startsWith('Total sugerido') && l.includes('$40700'))).toBe(true)
