...
import MotivosDiferenciaInventario from './motivos-diferencia-inventario.vue'

const MOTIVO_ID = 'motivo-1'
const BORRADO_EL = '2026-08-01T21:00:00.000Z'

interface MotivoDiferenciaInventarioFake {
  id: string
  nombre: string
  activo: boolean
  esFijo: boolean
...
function motivo(over: Partial<MotivoDiferenciaInventarioFake> = {}): MotivoDiferenciaInventarioFake {
  return {
    id: MOTIVO_ID,
...
function eliminado(over: Partial<MotivoDiferenciaInventarioFake> = {}): MotivoDiferenciaInventarioFake {
  return motivo({
...
let motivosBackend: MotivoDiferenciaInventarioFake[] = []
...
mockNuxtImport('useApiFetch', () => {
  return (url: string, opts?: { method?: string, body?: { nombre?: string } }) => {
    if (typeof url !== 'string' || !url.includes('/motivos-diferencia-inventario')) {
...
async function montar() {
  const wrapper = await mountSuspended(MotivosDiferenciaInventario)
...
describe('configuracion/motivos-diferencia-inventario — papelera: eliminar respeta el toggle', () => {
  beforeEach(() => {
  ...
  it('con "Ver eliminados" activo, borrar deja la fila visible como eliminada (no la saca de la lista)', async () => {
    ...
    expect(wrapper.text()).toContain('Error de conteo')
  ...
  it('con el toggle apagado, borrar SÍ saca la fila de la lista (comportamiento de siempre)', async () => {
    ...
    expect(wrapper.text()).not.toContain('Error de conteo')
...
describe('configuracion/motivos-diferencia-inventario — papelera: restaurar', () => {
  beforeEach(() => {
    motivosBackend = [eliminado()]
    reset()
  })
...
describe('configuracion/motivos-diferencia-inventario — papelera: colisión de nombre al restaurar', () => {
  beforeEach(() => {
    motivosBackend = [
      eliminado(),
      motivo({ id: 'motivo-vivo', nombre: 'Error de conteo' }),
    ]
    reset()
  })
  ...
  it('el 400 abre el modal con la sugerencia precargada y confirmar restaura CON ese nombre', async () => {
    ...
    expect(document.body.textContent).toContain(
      'Ya existe un motivo de diferencia activo con el nombre "Error de conteo".',
    )
    expect(campoNombre().value).toBe('Error de conteo 2')

    await confirmarEnModal('Restaurar')

    // El segundo POST sí llevó el nombre, y la fila revivió renombrada.
    expect(postsRestaurar).toHaveLength(2)
    expect(postsRestaurar[1]).toEqual({ id: MOTIVO_ID, nombre: 'Error de conteo 2' })
    expect(motivosBackend[0]!.eliminadoEl).toBeNull()
    expect(motivosBackend[0]!.nombre).toBe('Error de conteo 2')
    // Los dos conviven vivos en la tabla, que es el punto de toda la salida.
    expect(wrapper.text()).toContain('Error de conteo 2')
  ...
  it('si el usuario edita a un nombre TAMBIÉN tomado, vuelve el modal con la sugerencia siguiente', async () => {
    motivosBackend.push(
      motivo({ id: 'motivo-vivo-2', nombre: 'Error de conteo 2' }),
    )
    const wrapper = await montar()
    await activarVerEliminados(wrapper)
    await abrirRestaurarDeLaFila(wrapper)
    ...
    // Con "Error de conteo" y "Error de conteo 2" vivos, la primera
    // sugerencia salta al 3; el usuario lo pisa a mano con uno que está
    // ocupado.
    expect(campoNombre().value).toBe('Error de conteo 3')
    await escribirNombre('Error de conteo 2')
    await confirmarEnModal('Restaurar')
...
describe('configuracion/motivos-diferencia-inventario — papelera: la carrera de `cargar()` bajo toggles rápidos', () => {
  beforeEach(() => {
