 ...
 * era su TERCER uso, y `CLAUDE.md` manda extraer a la tercera. `elegirTipo` y
 * el click de radio siguen duplicados —van por la segunda— y por eso quedan
 * locales.
 */
describe('configuracion/recargos — perder la forma de importe avisa por los cuatro caminos', () => {
  ...
  async function elegirForma(valor: 'valor' | 'tramos') {
    const radio = dialogo()?.querySelector<HTMLElement>(`[role="radio"][value="${valor}"]`)
    expect(radio, `radio de forma "${valor}"`).toBeTruthy()
    radio!.click()
    await new Promise(r => setTimeout(r, 20))
  }
