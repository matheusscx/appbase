...
// Cómo dice la regla cuánto cobra. Es un eje distinto del `modo`: el modo elige
// la UNIDAD (plata o porcentaje) y esto elige la FORMA (un valor para toda la
// venta, o escalones según el monto). Solo aparece en los tipos que admiten las
// dos — ver `eligeForma`.
...
function abrirEditar(r: Regla) {
  ...
  form.value = {
    ...
    // cobrar por escalones, y el backend garantiza que no haya fila con las dos.
    formaImporte: r.tramos?.length ? 'tramos' as const : 'valor' as const,
  ...
  escalonesGuardados.value = r.tramos?.length ?? 0
  valorGuardado.value = !!(r.valorMonto ?? r.valorPorcentaje)
  nivelTocado.value = true
  drawerOpen.value = true
}
  ...
  try {
    ...
    if (cfg) {
      ...
      if (mostrarTramos.value) {
        // Todos los tipos de recargo con tramos miden MONTO de venta
        // (`recargo_por_monto_venta` y `recargo_metodo_pago`), así que el
        // umbral va siempre en `minimoMonto`. En descuentos no alcanza con
        // esto: ahí `por_mayor` mide cantidad y el body elige columna según el
        // tipo.
      ...
      <template #body>
        ...
        >
          ...
          <template v-if="config">
            ...
            <!-- Cómo cobra — solo en los tipos que admiten las dos formas. -->
            <UFormField v-if="eligeForma" label="Cómo cobra" required>
              <URadioGroup
