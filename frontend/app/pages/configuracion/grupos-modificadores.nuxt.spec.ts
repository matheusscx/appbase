...
//
// Réplica del molde de `descuentos.nuxt.spec.ts` para `grupos-modificadores`:
// la papelera (toggle "ver eliminados" + badge + botón Restaurar + modal de
...
function grupo(over: Partial<GrupoFake> = {}): GrupoFake {
  return {
...
function eliminado(over: Partial<GrupoFake> = {}): GrupoFake {
  return grupo({
...
// `null` = comportamiento normal (resuelve contra `gruposBackend`).
...
mockNuxtImport('useApiFetch', () => {
  return (url: string, opts?: { method?: string, body?: { nombre?: string } }) => {
    ...
    if (!url.includes('/grupos-modificadores')) {
      ...
      // y cualquier otra llamada ajena a este spec.
    ...
    if (method === 'POST' && url.endsWith('/restaurar')) {
      ...
      if (!g?.eliminadoEl) {
        return Promise.reject(
          errorApi(`Grupo de modificadores ${id} no está en la papelera`),
        )
      }
      const nombre = nombreNuevo ?? g.nombre
      const vivos = gruposBackend
        .filter(x => !x.eliminadoEl && x.grupoModificadorId !== id)
        .map(x => x.nombre)
      if (vivos.includes(nombre)) {
        return Promise.reject(
          errorApi(
            `Ya existe un grupo de modificadores activo con el nombre "${nombre}".`,
    ...
    const data = incluirEliminados
      ? gruposBackend
      : gruposBackend.filter(g => !g.eliminadoEl)
    return Promise.resolve(data.map(g => ({ ...g })))
  }
})

async function montar() {
  const wrapper = await mountSuspended(GruposModificadores)
...
describe('configuracion/grupos-modificadores — papelera: eliminar respeta el toggle', () => {
  ...
  it('con "Ver eliminados" activo, borrar deja la fila visible como eliminada (no la saca de la lista)', async () => {
    ...
    expect(gruposBackend[0]!.eliminadoEl).toBeTruthy()
    expect(wrapper.text()).toContain('Salsas')
  ...
  it('con el toggle apagado, borrar SÍ saca la fila de la lista (comportamiento de siempre)', async () => {
    ...
    expect(wrapper.text()).not.toContain('Salsas')

    wrapper.unmount()
  })
})

describe('configuracion/grupos-modificadores — papelera: restaurar', () => {
  beforeEach(() => {
    gruposBackend = [eliminado()]
    reset()
  })
  ...
  it('restaurar devuelve la fila al estado vivo sin recargar la página', async () => {
    ...
    expect(gruposBackend[0]!.eliminadoEl).toBeNull()
    expect(wrapper.text()).toContain('Salsas')
  ...
  it('dos clicks en Restaurar mandan UN solo POST (no un 404 encima del éxito)', async () => {
    ...
    expect(postsRestaurar).toEqual([{ id: GRUPO_ID, nombre: undefined }])
  ...
  it('un error SIN sugerencia no abre el modal de colisión: cierra y avisa', async () => {
    ...
    const wrapper = await montar()
    await activarVerEliminados(wrapper)
    gruposBackend[0]!.eliminadoEl = null
...
// La primera con la salida de colisión de nombre en `grupos-modificadores`.
...
describe('configuracion/grupos-modificadores — papelera: colisión de nombre al restaurar', () => {
  beforeEach(() => {
    gruposBackend = [
      eliminado(),
      grupo({ grupoModificadorId: 'grupo-vivo', nombre: 'Salsas' }),
    ]
    reset()
  })
  ...
  it('el 400 abre el modal con la sugerencia precargada y confirmar restaura CON ese nombre', async () => {
    ...
    expect(postsRestaurar).toEqual([{ id: GRUPO_ID, nombre: undefined }])
    expect(gruposBackend[0]!.eliminadoEl).toBe(BORRADO_EL)
    ...
    expect(document.body.textContent).toContain(
      'Ya existe un grupo de modificadores activo con el nombre "Salsas".',
    )
    expect(campoNombre().value).toBe('Salsas 2')

    await confirmarEnModal('Restaurar')

    // El segundo POST sí llevó el nombre, y la fila revivió renombrada.
    expect(postsRestaurar).toHaveLength(2)
    expect(postsRestaurar[1]).toEqual({ id: GRUPO_ID, nombre: 'Salsas 2' })
    expect(gruposBackend[0]!.eliminadoEl).toBeNull()
    expect(gruposBackend[0]!.nombre).toBe('Salsas 2')
    // Los dos conviven vivos en la tabla, que es el punto de toda la salida.
    expect(wrapper.text()).toContain('Salsas 2')
  ...
  it('si el usuario edita a un nombre TAMBIÉN tomado, vuelve el modal con la sugerencia siguiente', async () => {
    gruposBackend.push(
      grupo({ grupoModificadorId: 'grupo-vivo-2', nombre: 'Salsas 2' }),
    )
    const wrapper = await montar()
    await activarVerEliminados(wrapper)
    await abrirRestaurarDeLaFila(wrapper)
    ...
    // Con "Salsas" y "Salsas 2" vivos, la primera sugerencia salta al 3; el
    // usuario lo pisa a mano con uno que está ocupado.
    expect(campoNombre().value).toBe('Salsas 3')
    await escribirNombre('Salsas 2')
    await confirmarEnModal('Restaurar')
...
describe('configuracion/grupos-modificadores — papelera: la carrera de `cargar()` bajo toggles rápidos', () => {
  beforeEach(() => {
    // Uno vivo y otro ya eliminado: así el `GET` con el flag trae algo (Grupo
    // viejo) que el `GET` sin el flag no trae, y las dos respuestas son
    // distinguibles en el DOM.
    gruposBackend = [
      grupo(),
      eliminado({ grupoModificadorId: 'grupo-2', nombre: 'Grupo viejo' }),
    ]
    reset()
  })
  ...
  it('si la respuesta del primer toggle llega DESPUÉS que la del segundo, el listado final igual corresponde al último toggle', async () => {
    ...
    resolverSinEliminados(
      gruposBackend.filter(g => !g.eliminadoEl).map(g => ({ ...g })),
    )
    await new Promise(r => setTimeout(r, 20))
    resolverConEliminados(gruposBackend.map(g => ({ ...g })))
    await new Promise(r => setTimeout(r, 50))
