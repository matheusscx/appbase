...
//
// Réplica del molde de `causas-merma.nuxt.spec.ts` para `motivos-traslado`: la
...
import MotivosTraslado from './motivos-traslado.vue'

const MOTIVO_ID = 'motivo-1'
const BORRADO_EL = '2026-08-01T21:00:00.000Z'

interface MotivoTrasladoFake {
  id: string
  nombre: string
  activo: boolean
  esFijo: boolean
...
function motivo(over: Partial<MotivoTrasladoFake> = {}): MotivoTrasladoFake {
  return {
    id: MOTIVO_ID,
    nombre: 'Traslado a bodega Providencia',
    activo: true,
    esFijo: false,
    eliminadoEl: null,
...
function eliminado(over: Partial<MotivoTrasladoFake> = {}): MotivoTrasladoFake {
  return motivo({
...
let motivosBackend: MotivoTrasladoFake[] = []
...
mockNuxtImport('useApiFetch', () => {
  return (url: string, opts?: { method?: string, body?: { nombre?: string } }) => {
    if (typeof url !== 'string' || !url.includes('/motivos-traslado')) {
    ...
    if (method === 'POST' && url.endsWith('/restaurar')) {
      ...
      if (!m?.eliminadoEl) {
        return Promise.reject(
          errorApi(`Motivo de traslado ${id} no está en la papelera`),
        )
      }
      ...
      if (vivos.includes(nombre)) {
        return Promise.reject(
          errorApi(
            `Ya existe un motivo de traslado activo con el nombre "${nombre}".`,
...
async function montar() {
  const wrapper = await mountSuspended(MotivosTraslado)
...
describe('configuracion/motivos-traslado — papelera: eliminar respeta el toggle', () => {
  beforeEach(() => {
  ...
  it('con "Ver eliminados" activo, borrar deja la fila visible como eliminada (no la saca de la lista)', async () => {
    ...
    expect(wrapper.text()).toContain('Traslado a bodega Providencia')
  ...
  it('con el toggle apagado, borrar SÍ saca la fila de la lista (comportamiento de siempre)', async () => {
    ...
    expect(wrapper.text()).not.toContain('Traslado a bodega Providencia')
...
describe('configuracion/motivos-traslado — papelera: restaurar', () => {
  beforeEach(() => {
    motivosBackend = [eliminado()]
    reset()
  })
...
describe('configuracion/motivos-traslado — papelera: colisión de nombre al restaurar', () => {
  beforeEach(() => {
    motivosBackend = [
      eliminado(),
      motivo({ id: 'motivo-vivo', nombre: 'Traslado a bodega Providencia' }),
    ]
    reset()
  })
  ...
  it('el 400 abre el modal con la sugerencia precargada y confirmar restaura CON ese nombre', async () => {
    ...
    expect(document.body.textContent).toContain(
      'Ya existe un motivo de traslado activo con el nombre "Traslado a bodega Providencia".',
    )
    expect(campoNombre().value).toBe('Traslado a bodega Providencia 2')

    await confirmarEnModal('Restaurar')

    // El segundo POST sí llevó el nombre, y la fila revivió renombrada.
    expect(postsRestaurar).toHaveLength(2)
    expect(postsRestaurar[1]).toEqual({ id: MOTIVO_ID, nombre: 'Traslado a bodega Providencia 2' })
    expect(motivosBackend[0]!.eliminadoEl).toBeNull()
    expect(motivosBackend[0]!.nombre).toBe('Traslado a bodega Providencia 2')
    // Los dos conviven vivos en la tabla, que es el punto de toda la salida.
    expect(wrapper.text()).toContain('Traslado a bodega Providencia 2')
  ...
  it('si el usuario edita a un nombre TAMBIÉN tomado, vuelve el modal con la sugerencia siguiente', async () => {
    motivosBackend.push(
      motivo({ id: 'motivo-vivo-2', nombre: 'Traslado a bodega Providencia 2' }),
    )
    const wrapper = await montar()
    await activarVerEliminados(wrapper)
    await abrirRestaurarDeLaFila(wrapper)
    ...
    // Con "Traslado a bodega Providencia" y "... 2" vivos, la primera
    // sugerencia salta al 3; el usuario lo pisa a mano con uno que está
    // ocupado.
    expect(campoNombre().value).toBe('Traslado a bodega Providencia 3')
    await escribirNombre('Traslado a bodega Providencia 2')
    await confirmarEnModal('Restaurar')
...
describe('configuracion/motivos-traslado — papelera: la carrera de `cargar()` bajo toggles rápidos', () => {
  beforeEach(() => {
