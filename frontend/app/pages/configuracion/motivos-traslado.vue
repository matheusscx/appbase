...
const { verEliminados, restaurar, formatearBorradoPor } = usePapelera('motivos-traslado')

const motivos = ref<MotivoTraslado[]>([])
...
async function cargar() {
  ...
  const actual = (async () => {
    ...
    try {
      ...
      motivos.value = await useApiFetch<MotivoTraslado[]>(`${apiUrl}/motivos-traslado${query}`)
    }
    catch (e: unknown) {
...
function upsertLocal(saved: MotivoTraslado) {
...
function abrirEditar(motivo: MotivoTraslado) {
...
async function guardar() {
  ...
  try {
    ...
    const saved = isNew
      ? await useApiFetch<MotivoTraslado>(`${apiUrl}/motivos-traslado`, { method: 'POST', body })
      : await useApiFetch<MotivoTraslado>(`${apiUrl}/motivos-traslado/${editingId.value}`, {
          method: 'PATCH',
          body,
        })
    upsertLocal(saved)
...
function pedirEliminar(motivo: MotivoTraslado) {
...
async function eliminar(id: string) {
  try {
    await useApiFetch(`${apiUrl}/motivos-traslado/${id}`, { method: 'DELETE' })
...
const columns: TableColumn<MotivoTraslado>[] = [
...
<template>
  <div class="space-y-6">
    <CrudPageHeader
      ...
      description="Tipifica por qué se mueve stock entre ubicaciones propias. Los motivos fijos del sistema no se editan."
    >
      <template #actions>
      ...
      <template #body>
        ...
        >
          ...
          >
            <UInput
              v-model="form.nombre"
              placeholder="Ej: Traslado a bodega Providencia"
              autofocus
    ...
    <CrudModal
      ...
      message="¿Eliminar este motivo de traslado? Podés recuperarlo desde «Ver eliminados»."
    ...
    <CrudModal
      ...
      message="¿Restaurar este motivo de traslado? Volverá a aparecer en el listado y podrá usarse de nuevo."
