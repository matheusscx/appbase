...
function vistaInicial(): ReporteTab {
  const vista = queryStringValue(route.query.vista)
  if (vista === 'trabajadores') return 'trabajadores'
  // Compat: URLs antiguas usaban ?tab=trabajadores en /propinas/reportes
  if (queryStringValue(route.query.tab) === 'trabajadores') return 'trabajadores'
  return 'resumen'
}

const vista = ref<ReporteTab>(vistaInicial())
...
const tipoSeleccionado = computed({
  get: () => tipoTrabajadorParaSelect(filtros.value.tipoGarzon),
  set: (value: TipoTrabajadorSelect) => {
    filtros.value.tipoGarzon = tipoTrabajadorDesdeSelect(value)
  },
})
...
async function cargarVista(options: { force?: boolean } = {}) {
  let key: string
  try {
  ...
  if (!options.force) {
    const cached = reportesApi.cache.get(vista.value, key)
    if (cached) {
      asignarCache(vista.value, cached)
      return
    }
  }
...
function queryReportes(extra: Record<string, string | undefined> = {}) {
  return {
    tab: 'reportes',
    vista: vista.value,
...
async function aplicar() {
  ...
  await router.replace({ query: queryReportes() })
  await cargarVista({ force: true })
}

watch(vista, async (value) => {
  await router.replace({ query: { ...route.query, tab: 'reportes', vista: value } })
  await cargarVista()
})
...
<template>
  <div class="space-y-6">
    ...
    <UTabs v-model="vista" :items="vistaItems" :content="false" />
    ...
    <template v-else-if="vista === 'resumen' && resumen">
    ...
    <template v-else-if="vista === 'trabajadores' && trabajadores">
      <UAlert
