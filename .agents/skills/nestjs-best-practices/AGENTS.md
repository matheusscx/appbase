...
When multiple database operations must succeed or fail together, wrap them in a transaction. This prevents partial updates that leave your data in an inconsistent state. En este repo se abren con `db.transaccion(fn)` — ver el override al tope de SKILL.md.
