...
describe('Papelera (e2e) — familia softDelete(): descuentos, recargos, impuestos, terceros, cajones, garzones, turnos, impresoras', () => {
  ...
  it('turnos: el 400 de colisión trae un nombre libre, y restaurar con ese nombre funciona', async () => {
    const nombre = `Turno sugerencia E2E ${Date.now()}`;
    const crear = async (n: string) => {
      const res = await request(app.getHttpServer())
        .post('/api/turnos')
        .set('Authorization', `Bearer ${tokenAdmin}`)
        .send({ nombre: n, horaInicio: '12:00', horaFin: '14:00' });
      expect(res.status).toBe(201);
      return (res.body as RecursoConAuditoria).id;
    };
