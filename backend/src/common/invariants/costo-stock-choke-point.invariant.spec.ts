...
//
// Límite conocido (revisión de rama 2026-09-06): esto solo mira backticks. SQL
// armado con comillas simples evade los tres tests de acá abajo. Es
// preexistente y coherente con cómo el repo escribe SQL (siempre template
// literals, nunca comillas simples) — no se corrige porque hacerlo exigiría un
// parser de SQL, no una regex, para no reventar de falsos positivos sobre
// cualquier string con comillas simples que no sea SQL.
  ...
  it('nadie escribe stock_ubicacion, lote_ubicacion ni item_unidad.ubicacion_id fuera de inventario.service', () => {
    ...
    for (const file of archivos) {
      ...
      const sospechoso =
        ...
        // faltaba acá (revisión de rama 2026-09-06): las dos puertas de arriba
        // solo cazaban su SQL crudo (`INSERT`/`UPDATE` sobre `ubicacion_id`),
        // no un `save({ ubicacionId })` por repositorio, que movía una unidad
        // serializada de lugar sin pasar por el kardex y el invariante seguía
        // verde. Esto se busca sobre el archivo entero, no sobre los literales,
        // porque no es SQL.
