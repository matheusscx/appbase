...
export class DescuentosService {
  ...
  async restaurar(
    ...
    try {
      ...
      // también donde no lo enumeramos. Mismo patrón que cajones y motivos-baja.
   ...
   * `UPDATE` deja una fila de `item_descuentos` con una regla de nivel venta, que
  ...
  private async validarCambioDeNivel(
    ...
    await this.db.query(
      `SELECT 1 FROM descuentos
        WHERE descuento_id = $1 AND tenant_id = $2
        FOR UPDATE`,
