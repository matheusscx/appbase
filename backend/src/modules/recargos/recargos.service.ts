...
export class RecargosService {
    ...
    const escritura = this.db.transaccion(async (manager) => {
      ...
      if (dto.metodoPagoIds !== undefined) {
        ...
        if (dto.metodoPagoIds.length) {
          ...
          // compuesta `(recargo_id, metodo_pago_id)`, así que un método que ya estuvo
          ...
          await manager.query(
            `INSERT INTO recargo_metodo_pago (recargo_id, metodo_pago_id, creado_el, actualizado_el)
             VALUES ${dto.metodoPagoIds.map((_, i) => `($1, $${i + 2}, NOW(), NOW())`).join(', ')}
             ON CONFLICT (recargo_id, metodo_pago_id)
  ...
  private async validarCambioDeNivel(
    ...
    if (asociados > 0) {
      throw new BadRequestException(
        `No se puede pasar a nivel venta: ${asociados} ítem(s) todavía tienen este recargo asociado (incluidos los que estén en la papelera). Quitá la asociación primero.`,
      );
    }
  }
