...
describe('RecargosService', () => {
  ...
  describe('update', () => {
    ...
    it('revives the soft-stamped row instead of inserting a new one', async () => {
      ...
      expect(sql).toMatch(
        /ON CONFLICT \(recargo_id, metodo_pago_id\)\s+DO UPDATE SET eliminado_el = NULL/,
      );
      expect(params).toEqual(['r-1', 'mp-5']);
    });
  ...
  describe('nivel', () => {
    ...
    it('el guard toma el `FOR UPDATE` de la fila de la regla ANTES de contar', async () => {
      ...
      // `item_recargos` no serializa contra el cambio de nivel.
      expect(sqls[iLock]).toContain('FROM recargos');
      expect(sqls[iLock]).toContain('recargo_id = $1');
    });
